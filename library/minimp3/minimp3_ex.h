...
extern "C" {
    ...
    int mp3dec_load_buf(mp3dec_t* dec, const uint8_t* buf, size_t buf_size, mp3dec_file_info_t* info, MP3D_PROGRESS_CB progress_cb, void* user_data, int max_buffer_size);
    int mp3dec_load_cb(mp3dec_t* dec, mp3dec_io_t* io, uint8_t* buf, size_t buf_size, mp3dec_file_info_t* info, MP3D_PROGRESS_CB progress_cb, void* user_data, int max_buffer_size);
...
int mp3dec_load_buf(mp3dec_t* dec, const uint8_t* buf, size_t buf_size, mp3dec_file_info_t* info, MP3D_PROGRESS_CB progress_cb, void* user_data, int max_buffer_size)
{
    return mp3dec_load_cb(dec, 0, (uint8_t*)buf, buf_size, info, progress_cb, user_data, max_buffer_size);
}
...
int mp3dec_load_cb(mp3dec_t* dec, mp3dec_io_t* io, uint8_t* buf, size_t buf_size, mp3dec_file_info_t* info, MP3D_PROGRESS_CB progress_cb, void* user_data, int max_buffer_size)
{
    ...
    if (max_buffer_size != 0 && max_buffer_size <= (int)allocated)
        allocated = max_buffer_size;
...
{
    int ret = mp3dec_load_buf(dec, map_info->buffer, map_info->size, info, progress_cb, user_data, 0);
    mp3dec_close_file(map_info);
    return ret;
}
