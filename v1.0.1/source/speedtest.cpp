...
void Speed_test_main(void)
{
	...
	Draw("Speed : " + std::to_string((speed_test_result / (1024 * 1024)) * 8) + "Mbps", 0.0f, 60.0f, 1.0f, 1.0f, 0.25f, 0.0f, 1.0f, 1.0f);
	...
	while (share_speed_test_thread_run)
	{
		...
		else
		{
			...
			for (int i = 0; i <= 9; i++)
			{
				...
				share_function_result = httpcOpenContext(&speed_test_httpc, HTTPC_METHOD_GET, speed_test_url[speed_test_data_size].c_str(), 0);
				if (share_function_result != 0)
				{
					i = 100;
					//wifi_auth_app_log_num_return = Share_app_log_save("Speedtest/Network", "httpcOpenContext...", function_result, false);
				}

				share_function_result = httpcSetSSLOpt(&speed_test_httpc, SSLCOPT_DisableVerify);
				if (share_function_result != 0)
				{
					i = 100;
					//wifi_auth_app_log_num_return = Share_app_log_save("Speedtest/Network", "httpcSetSSLOpt...", function_result, false);
				}

				share_function_result = httpcSetKeepAlive(&speed_test_httpc, HTTPC_KEEPALIVE_ENABLED);
				if (share_function_result != 0)
				{
					i = 100;
					//wifi_auth_app_log_num_return = Share_app_log_save("Speedtest/Network", "httpcSetKeepAlive...", function_result, false);
				}
				...
				share_function_result = httpcBeginRequest(&speed_test_httpc);
				if (share_function_result != 0)
				{
					i = 100;
					//wifi_auth_app_log_num_return = Share_app_log_save("Speedtest/Network", "httpcBeginRequest...", function_result, false);
				}

				osTickCounterUpdate(&speed_test_time);
				share_function_result = httpcDownloadData(&speed_test_httpc, speed_test_httpc_buffer, 0x1000000, &speed_test_download_size_u32);
				osTickCounterUpdate(&speed_test_time);
				...
				speed_test_result = speed_test_total_download_size / (speed_test_total_download_time / 1000);

				if (share_function_result != 0)
				{
					i = 100;
					//wifi_auth_app_log_num_return = Share_app_log_save("Speedtest/Network", "httpcDownloadData...", function_result, false);
				}
