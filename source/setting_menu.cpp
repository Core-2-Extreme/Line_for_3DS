...
void Sem_init(void)
{
	Log_log_save(sem_init_string, "Initializing...", 1234567890, s_debug_slow);


	Draw_progress("0/0 [Sem] Starting threads...");
	sem_check_update_thread_run = true;
	sem_worker_thread_run = true;
	sem_check_update_thread = threadCreate(Sem_check_update_thread, (void*)(""), STACKSIZE, 0x28, -1, false);
	sem_worker_thread = threadCreate(Sem_worker_thread, (void*)(""), STACKSIZE, 0x32, -1, false);

	Sem_resume();
	sem_already_init = true;
	Log_log_save(sem_init_string, "Initialized.", 1234567890, s_debug_slow);
}

void Sem_exit(void)
{
	Log_log_save(sem_exit_string, "Exiting...", 1234567890, s_debug_slow);
	...
	if (failed)
		Log_log_save(sem_exit_string, "[Warn] Some function returned error.", 1234567890, s_debug_slow);

	Log_log_save(sem_exit_string, "Exited.", 1234567890, s_debug_slow);
}
...
void Sem_main(void)
{
	...
	if (sem_selected_menu_mode >= 1 && sem_selected_menu_mode <= 7)
	{
		draw_y = 0.0;
	...
	if (sem_selected_menu_mode == 0)
	{
		...
		for (int i = 0; i < 7; i++)
		{
			...
			Draw(sem_msg[i], 0, 0.0, draw_y + sem_y_offset - 2.5, 0.75, 0.75, text_red, text_green, text_blue, text_alpha);
			draw_y += 25.0;
		}
	}
	...
	{
		//Buffer size
		buffer_size[0] = Line_query_buffer_size(LINE_HTTPC_BUFFER);
		buffer_size[1] = Line_query_buffer_size(LINE_FS_BUFFER);
		buffer_size[2] = Line_query_buffer_size(LINE_SEND_FS_CACHE_BUFFER);
		buffer_size[3] = Line_query_buffer_size(LINE_SEND_FS_BUFFER);
		buffer_size[4] = Spt_query_buffer_size(SPT_HTTPC_BUFFER);
		buffer_size[5] = Imv_query_buffer_size(IMV_HTTPC_BUFFER);
		buffer_size[6] = Imv_query_buffer_size(IMV_FS_BUFFER);
		buffer_size[7] = Mup_query_buffer_size(MUP_FS_OUT_BUFFER);
		buffer_size[8] = Mup_query_buffer_size(MUP_FS_IN_BUFFER);
		...
		for (int i = 0; i < 9; i++)
		{
			if (draw_y + sem_y_offset >= -30 && draw_y + sem_y_offset <= 240)
			{
				Draw(sem_msg[29 + i] + std::to_string(buffer_size[i] / 1024.0 / 1024.0).substr(0, 5) + "MB", 0, (draw_x - 10.0), draw_y + sem_y_offset, 0.5, 0.5, red[i], green[i], blue[i], alpha[i]);
				Draw_texture(Square_image, weak_red_tint, 0, draw_x, draw_y + sem_y_offset + 22.5, 300.0, 5.0);
				Draw_texture(Square_image, white_or_black_tint, 0, (((buffer_size[i] - 0x40000) / (buffer_max_size[i] - 0x40000)) * 295.0) + draw_x, draw_y + sem_y_offset + 15.0, 5.0, 20.0);
			}
			draw_y += 40.0;
		}
	}
	...
	while (sem_worker_thread_run)
	{
		...
		else if (sem_load_external_font_request)
		{
			for (int i = 0; i < 46; i++)
			{
				if (sem_load_external_font[i] && !sem_loaded_external_font[i])
				{
					log_num = Log_log_save(sem_worker_thread_string, "Loading texture (" + Exfont_query_font_name(i) + "_font.t3x)...", 1234567890, false);
		...
		else if (sem_reload_msg_request)
		{
			for (int i = 0; i < 8; i++)
			{
				log_num = Log_log_save(sem_worker_thread_string, "File_load_from_rom(" + load_file_name[i] + s_setting[0] + ".txt)...", 1234567890, false);
				result = File_load_from_rom(load_file_name[i] + s_setting[0] + ".txt", fs_buffer, 0x2000, &read_size, "romfs:/gfx/msg/");
				Log_log_add(log_num, result.string, result.code, false);
		...
		else if (sem_delete_line_img_cache_request)
		{
			...
			log_num = Log_log_save(sem_worker_thread_string, "File_read_dir(/Line/images/)...", 1234567890, false);
			...
			if (result.code == 0)
			{
				for (int i = 0; i < num_of_files; i++)
				{
					if (file_type[i] == "file")
					{
						log_num = Log_log_save(sem_worker_thread_string, "File_file_delete(" + file_name[i] + ")...", 1234567890, false);
...
void Sem_check_update_thread(void* arg)
{
	...
	while (sem_check_update_thread_run)
	{
		if (sem_check_update_request || sem_dl_file_request)
		{
			...
			{
				if (sem_check_update_request)
					operation = "Checking for update...";
				else if (sem_dl_file_request)
					operation = "Downloading file...";
				else
					operation = "...";

				check_update_log_num_return = Log_log_save(sem_check_update_string, operation, 1234567890, false);
				check_update_result = Httpc_dl_data(url, httpc_buffer, 0x300000, &downloaded_size, &status_code, true, &last_url, false, 100);
				...
				else
				{
					if (sem_check_update_request)
					{
						...
						{
							new_version_available = false;
							Log_log_save(sem_check_update_string, sem_msg[45], 1234567890, false);
						}
					...
					else if (sem_dl_file_request)
					{
						...
						if (sem_selected_edition_num == 0)
						{
							check_update_log_num_return = Log_log_save(sem_check_update_string, "Save_to_file...", 1234567890, false);
